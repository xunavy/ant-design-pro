...
interface MultiKeywordStatsProps {
  data: { coOccurrences: MultiFieldStat[]; comparison: { keyword: string; count: number }[] };
}

const MultiKeywordStats: React.FC<MultiKeywordStatsProps> = ({ data }) => {
  // 表格列配置
  const coOccurrencesColumns = [
    { title: '关键字组合', dataIndex: 'keywords', key: 'keywords', render: (keywords: string[]) => keywords.join(' & ') },
    { title: '共现次数', dataIndex: 'cooccurrenceCount', key: 'cooccurrenceCount', align: 'right' },
  ];
